...
```bash
node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
@/workspace/.codex/get-shit-done/references/thinking-models-planning.md
...
```bash
VALID=$(node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
