...
```bash
INIT=$(node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "{{GSD_ARGS}}")
...
```bash
RESEARCHER_MODEL=$(node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}")
```
