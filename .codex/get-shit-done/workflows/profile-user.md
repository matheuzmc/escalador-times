...
Key references:
- @/workspace/.codex/get-shit-done/references/ui-brand.md (display patterns)
- @/workspace/.codex/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @/workspace/.codex/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="/workspace/.codex/get-shit-done/USER-PROFILE.md"
...
```bash
cp "/workspace/.codex/get-shit-done/USER-PROFILE.md" "/workspace/.codex/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at /workspace/.codex/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `/workspace/.codex/get-shit-done/references/user-profiling.md`
...
Reference: @/workspace/.codex/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to /workspace/.codex/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global AGENTS.md" -- "Add profile to /workspace/.codex/AGENTS.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at /workspace/.codex/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at /workspace/.codex/commands/gsd/dev-preferences.md"

**For AGENTS.md profile section (if selected):**

```bash
node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node /workspace/.codex/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to /workspace/.codex/AGENTS.md"
...
```bash
BACKUP_PATH="/workspace/.codex/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    /workspace/.codex/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   /workspace/.codex/commands/gsd/dev-preferences.md
  ✓ AGENTS.md section       ./AGENTS.md
  ✓ Global AGENTS.md        /workspace/.codex/AGENTS.md
```
