...
```bash
node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/workspace/.codex/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
if [ -d "$TASKS_DIR" ]; then
  ...
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /workspace/.codex/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /workspace/.codex/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
