...
- **Global scope**: `/workspace/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspace/.codex/notes/`)
...
2. Glob `/workspace/.codex/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `/workspace/.codex/notes/` — works in any directory
