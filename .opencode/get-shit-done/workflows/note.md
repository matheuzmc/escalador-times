...
- **Global scope**: `/workspace/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspace/.opencode/notes/`)
...
2. Glob `/workspace/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/workspace/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspace/.opencode/notes/` — works in any directory
