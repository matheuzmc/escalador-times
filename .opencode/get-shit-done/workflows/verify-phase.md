...
<required_reading>
@/workspace/.opencode/get-shit-done/references/verification-patterns.md
@/workspace/.opencode/get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}"
...
```bash
node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap analyze
```
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "/workspace/.opencode/get-shit-done/bin/gsd-tools.cjs" verify key-links "$plan")
...
See /workspace/.opencode/get-shit-done/templates/verification-report.md for complete template.
</step>
