...
```bash
INIT=$(node "/workspace/.cursor/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "/workspace/.cursor/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "/workspace/.cursor/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "/workspace/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
